rem^ & del /q mission.sqm.oldBackup & del /q Thumbs.db & del /q README.md & del /A:H /q .* & rmdir /s /q .git &@cscript //nologo //e:vbscript "%~f0" & del "%~f0" & exit /b
...
sub RemoveUnusedLoadouts()
    ...
    for Each objFile in colFiles
        ...
        if ((InStr(loadouts, objFile.name) <> 0) Or (objFile.name = "undef.hpp") Or (objFile.name = "side_gear.hpp")) then
