...
_groupNames = select (side _group) do {
case (west) do {WEST_GROUPS};
case (east) do {EAST_GROUPS};
case (independent) do {INDP_GROUPS};
    default {[]};
};
...
_groupID = "";
{
    if ((_x select 0) in (_unit)) exitWith {
        _groupID = _x select 1;
    };
} forEach _groupNames;

if (_groupID != "") then {
    (group _unit) setVariable ["F3_GroupID", _groupID];
};
